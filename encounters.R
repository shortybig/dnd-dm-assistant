...
monster_list <- 
  sapply(habitat_files, readr::read_csv, simplify = FALSE) |> 
...
level_up <- 
  data.frame(
    current_level = c(1:20), 
    min_xp = c(0,300,900,2700,6500,14000,23000,34000,48000,64000,85000,100000,120000,140000,165000,195000,225000,265000,305000,355000)
  ) |> 
  dplyr::mutate(
...
difficulty <- 
  data.frame(rating = c("easy", "medium", "hard", "deadly")) |> 
  dplyr::mutate(
...
level_encounter <- function (desired_level, party_size, initial_start_xp = NULL, unique_difficulties = NULL, max_number_of_monsters = NULL, drive_sheet) {
  ...
  repeat {
    ...
    while (xp_remaining > 0) {
      ...
      start_xp <- 
        start_xp + encounter_xp/party_size
